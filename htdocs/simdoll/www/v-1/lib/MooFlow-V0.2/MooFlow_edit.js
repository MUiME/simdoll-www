...
var MooFlow = new Class({
	...
	getElements: function(el){
		...
		$$(els).each(function(el){
			var hash = $H(el.getElement('img').getProperties('src','title','alt','longdesc', 'width', 'height'));
			if(el.get('tag') == 'a') hash.combine(el.getProperties('href','rel','target'));
			this.master['images'].push(hash.getClean());
			this.arr_img.push([el.href, el.getElement('img').title]);
			el.dispose(); 
		}, this);
		this.clearMain();
	},
	...
	getBlankImgArr: function(){
		var els = [];
        this.master.images.each(function(){ 
			els.push(this.options.blankImgPath); 
		}, this);
		return els;
	},
	...
	addLoader: function(){
		...
		new Fx.Tween(this.MooFlow, {
			'duration': 800,
			'onComplete': this.preloadBlankImg.bind(this)
		}).start('opacity', 1);
	},
	
	preloadBlankImg: function(){
		this.index = this.options.startIndex;
		this.iL = this.master.images.length-1;
		this.oW = this.MooFlow.getSize().x;
		this.sz = this.oW * 0.5;
		this.x = 0; this.zI = this.iL;
		
		this.master.images.each(function(obj){
			...
			this.configImg(obj);
			obj['loading'] = new Element('div').setStyles({'position':'absolute', 'top':'19%', 'left':'42%', 'width':'17%', 'height':'12%'}).inject(obj['con']);
			new Element('img', {'src':this.options.loadingImgPath}).setStyles({'width':'100%', 'height':'100%'}).inject(obj['loading']);
		}, this);

		if(this.options.blankImgPath){
			this.loadedBlankImages = new Asset.images(this.getBlankImgArr(), {
				onProgress: this.createBlankImg.bind(this),
	...
	createBlankImg: function(counter, i){
		var obj = this.getCurrent(i);
		var img = this.loadedBlankImages[i];
		img.setStyles({'vertical-align':'bottom', 'width':'100%', 'height':'50%'}).inject(obj['con']);
	...
	loaded: function(){
		new Fx.Tween(this.loader, {
			'duration': 800,
			'onComplete': function(){ this.loader.dispose(); }.bind(this)
		}).start('opacity', 1);
	},
	...
	createUI: function(){
		...
		this.autoPlay_stop = new Element('a',{'class':'stop','events': {'click':this.stop.bind(this)}});
		this.autoPlay_stop.setStyle('display','none');
		this.autoPlay_play = new Element('a',{'class':'play','events': {'click':this.play.bind(this)}})
		...
		if(this.options.useAutoPlay){
			this.autoPlayCon.adopt(this.autoPlay_stop, this.autoPlay_play);
		}
	...
	update: function(e){
		...
		if(this.options.useSlider){ 
			...
			var w_knob;
			if(this.slider.getSize().x/this.iL < 17){ w_knob = 17; }
			else{ w_knob = this.slider.getSize().x/this.iL; }
			this.knob.setStyle('width',w_knob);
			
			if(!this.sli){
				this.sli = new SliderEx(this.slider, this.knob, {steps: this.iL, mode: 'horizontal'}).set(this.index);
				if(this.knob.offsetWidth < this.slider.offsetWidth){
	...
	setScreen: function(){
		if(this.isFull = !this.isFull){
			...
			window.scrollTo(0, 0);
			this.keyESC = function(e){ if(e.code == 27){ e.stop(); this.setScreen(); } }.bind(this);
			document.addEvent('keydown', this.keyESC);
			
			this.holder = new Element('div').inject(this.MooFlow,'after');
			this.MooFlow.wraps(new Element('div').inject(document.body));
			this.MooFlow.setStyles({'background-color':'white','position':'absolute','z-index':'100','top':'0','left':'0','width':window.getSize().x,'height':window.getSize().y});
			...
			if(this.scrolly == 0){ window.scrollTo(0, 1); }
			window.scrollTo(0, this.scrolly);
			document.removeEvent('keydown', this.keyESC);
			
			this.MooFlow.wraps(this.holder);
			window.removeEvent('resize', this._initResize);
			delete this.holder, this._initResize;
			this.MooFlow.setStyles({'background-color':this.options.bgColor,'position':'relative','z-index':'','top':'','left':'','width':'','height':this.MooFlow.retrieve('height')});
	...
	stop: function(){
		this.autoPlay_stop.setStyle('display','none');
		this.autoPlay_play.setStyle('display','block');
		$clear(this.autoPlay);
		this.isAutoPlay = false;
		this.fireEvent('autoStop');
	},
	play: function(){
		this.autoPlay_stop.setStyle('display','block');
		this.autoPlay_play.setStyle('display','none');
	...
	process: function(x){
		this.x = x; this.zI = this.iL;
		this.master.images.each(function(el){ this.configImg(el); }, this); 
	},
	configImg: function(el){
		var z,W,H,x=this.x,foc=this.foc,f=this.factor,sz=this.sz,oW=this.oW,offY=this.offY,div,elh,elw;
		div = el.div.style; 
		elw = el.width;
		elh = el.height; 
		if(x>-foc*6 && x<foc*6){
			...
			div.width = W + 'px';
			div.zIndex = x < 0 ? this.zI++ : this.zI--;
			div.display = 'block';
		} else {
			div.display = 'none';
		}
...
var SliderEx = new Class({
	...
	setSliderDimensions: function(){
		var offset; var mode = this.options.mode;
		if(mode == 'vertical'){ offset = 'offsetHeight'; }
		else if(mode == 'horizontal'){ offset = 'offsetWidth'; }
